...
class SourceCreate(BaseModel):
    name: str
    source_type: str  # "twitter" | "wechat"
    handle: str
...
def create_source(body: SourceCreate, db: Session = Depends(get_db)):
    if body.source_type not in ("twitter", "wechat"):
        raise HTTPException(status_code=400, detail="source_type must be 'twitter' or 'wechat'")
    source = Source(
        id=str(uuid.uuid4()),
        name=body.name,
